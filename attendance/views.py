...
def home(request):
    ...
    if request.user.profile.user_type == 1:
        ...
        classes = Class.objects.filter(
            assigned_faculty=request.user.profile).count()
        students = ClassStudent.objects.filter(classIns__in=Class.objects.filter(
            assigned_faculty=request.user.profile).values_list('id')).count()
...
def delete_department(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            resp['status'] = 'success'
            messages.success(
                request, 'Department has been deleted successfully.')
...
def manage_course(request, pk=None):
    ...
    if pk == None:
        ...
        department = Department.objects.filter(
            Q(status=1) or Q(id=course.id)).all()
    else:
        department = Department.objects.filter(status=1).all()
        course = {}
...
def manage_faculty(request, pk=None):
    if pk == None:
        ...
        department = Department.objects.filter(
            Q(status=1) or Q(id=faculty.id)).all()
    else:
        department = Department.objects.filter(status=1).all()
        faculty = {}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if profile is None:
                ...
                form2 = UpdateProfileMeta(
                    request.POST, request.FILES, instance=profile)
                if form2.is_valid():
                    form2.save()
                    resp['status'] = 'success'
                    messages.success(
                        request, 'Faculty has been save successfully.')
                else:
...
def manage_class_student(request, classPK=None):
    if classPK is None:
        ...
        students = Student.objects.exclude(id__in=ClassStudent.objects.filter(
            classIns=_class).values_list('student').distinct()).all()
        context['students'] = students
...
def delete_class_student(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            resp['status'] = 'success'
            messages.success(
                request, 'Student has been deleted from Class successfully.')
        except Exception as e:
            raise print(e)
    return HttpResponse(json.dumps(resp), content_type="application/json")
...
def save_student(request):
    ...
    if form.is_valid():
        ...
        resp['status'] = 'success'
        messages.success(
            request, 'Student Details has been saved successfully')
    else:
        for field in form:
            for error in field.errors:
                resp['msg'] += str(error + '<br>')
...
def delete_student(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            resp['status'] = 'success'
            messages.success(
                request, 'Student Details has been deleted successfully.')
        except Exception as e:
            raise print(e)
    return HttpResponse(json.dumps(resp), content_type="application/json")
...
def attendance(request, classPK=None, date=None):
    ...
    students = Student.objects.filter(id__in=ClassStudent.objects.filter(
        classIns=_class).values_list('student')).all()
    context['page_title'] = "Attendance Management"
    context['class'] = _class
    context['date'] = date
    att_data = {}
